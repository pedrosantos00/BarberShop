...
import { BarberStoreService } from '../Services/barber-store.service';
...
export class AuthGuard implements CanActivate {
  ...
  constructor(
    ...
    private tokenInfo: BarberStoreService
  ) { }
  ...
  canActivate(
    ...
    this.tokenInfo.getRole().subscribe(val => {
      // If the role is not available from the token, get it from the BarberStoreService and check the role
      this.role = val;
      return this.checkRole(expectedRole);
    });
  ...
  checkRole(expectedRole: string[] | string): boolean {
    // Check if the expected role matches the Barber's role
    if (Array.isArray(expectedRole)) {
      if (expectedRole.includes(this.role)) {
        // If the expected role is an array, check if the Barber's role is included in the expected roles
        return true;
      }
